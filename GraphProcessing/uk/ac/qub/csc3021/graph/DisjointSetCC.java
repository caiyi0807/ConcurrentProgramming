...
public class DisjointSetCC {
    private static class DSCCRelax implements Relax {
        ...
        private AtomicIntegerArray ranks;

        DSCCRelax(AtomicIntegerArray parent_,AtomicIntegerArray ranks_) {
            this.parent = parent_;
            this.ranks=ranks_;
        }
...
//        }
        ...
        private boolean union(int x, int y) {
            ...
            if(px==py)
                return false;
            if (ranks.get(py) > ranks.get(px))
                parent.set(px,py);
            else if (ranks.get(py) < ranks.get(px))
                parent.set(py,px);
            else {
                parent.set(py,px);
                int i = ranks.get(px);
